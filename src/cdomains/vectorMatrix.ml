...
module ConvenienceOps (A : RatOps) = struct
  let ( *: ) = A.mul
  let ( +: ) = A.add
  let ( -: ) = A.sub
  let ( /: ) = A.div
...
module type AbstractVector = functor (A : RatOps) -> sig
...
(** Some functions inside have the suffix _with, which means that the function has side effects. *)
module type AbstractMatrix = functor (A : RatOps) (V : AbstractVector) -> sig
  ...
  functor
    ...
    struct
      ...
      let show t =
        ...
        let rec list_str l =
          match l with [] -> "]" | x :: xs -> A.to_string x ^ " " ^ list_str xs
        in
        "[" ^ list_str t ^ "\n"

      let keep_vals v n =
        if n >= Array.length v then v
        else Array.filteri (fun i x -> i < n) v (* TODO: take? *)

      let compare_length_with v len = Int.compare (Array.length v) len

      let remove_val v n =
        if n >= Array.length v then failwith "n outside of Array range"
        else
          Array.init
            (Array.length v - 1)
            (fun i -> if i < n then Array.get v i else Array.get v (i + 1))
      (* TODO: remove_at? *)

      let set_val_with v n new_val =
        if n >= Array.length v then failwith "n outside of Array range"
        else Array.set v n new_val

      let set_val v n new_val =
        let copy = copy v in
        set_val_with copy n new_val;
        copy

      let insert_val n new_val v =
        if n > Array.length v then failwith "n too large"
        else
          Array.init
            (Array.length v + 1)
            (fun i ->
               if i < n then Array.get v i
               else if i = n then new_val
               else Array.get v (i - 1))
      (* insert? *)

      let apply_with_c f c v = Array.map (fun x -> f x c) v
      let zero_vec n = Array.make n A.zero
      let nth = Array.get

      let map2i f v1 v2 =
        let f' i = uncurry (f i) in
        Array.mapi f' (Array.combine v1 v2)
      (* TODO: iter2i? *)

      let map2i_with f v1 v2 = Array.iter2i (fun i x y -> v1.(i) <- f i x y) v1 v2
      let find2i f v1 v2 = Array.findi (uncurry f) (Array.combine v1 v2)
      (* TODO: iter2i? *)
...
module ArrayMatrix : AbstractMatrix =
  functor
    ...
    struct
      ...
      let show x =
        if M.tracing then M.trace "arrayMatrix" "show";
        Array.fold_left ( ^ ) "" (Array.map (fun v -> V.show @@ V.of_array v) x)

      let empty () =
        if M.tracing then M.trace "arrayMatrix" "empty";
      ...
      let copy m =
        ...
        Array.iteri (fun i x -> Array.blit x 0 cp.(i) 0 (num_cols m)) m;
        cp
      ...
      let append_row m row =
        if M.tracing then M.trace "arrayMatrix" "append_row";
      ...
      let get_row m n =
        if M.tracing then M.trace "arrayMatrix" "get_row";
        V.of_array m.(n)

      let remove_row m n =
        if M.tracing then M.trace "arrayMatrix" "remove_row";
        let new_matrix = make_matrix (num_rows m - 1) (num_cols m) A.zero in
        if not @@ is_empty new_matrix then
          if n = 0 then Array.blit m 1 new_matrix 0 (num_rows m - 1)
          else (
            Array.blit m 0 new_matrix 0 n;
            if n <> num_rows m - 1 then
              Array.blit m (n + 1) new_matrix n (num_rows new_matrix - n));
        new_matrix
      ...
      let append_matrices m1 m2 =
        if M.tracing then M.trace "arrayMatrix" "append_matrices";
        Array.append m1 m2
      ...
      let del_col m j =
        if M.tracing then M.trace "arrayMatrix" "del_col";
        if is_empty m then m
        else
          let new_matrix =
            Array.make_matrix (num_rows m) (num_cols m - 1) A.zero
          in
          for i = 0 to num_rows m - 1 do
                  ...
                  if m.(k).(j) <>: A.zero then (
                    ...
                    for l = 0 to num_rows - 1 do
                      if l <> i && m.(l).(j) <>: A.zero then (
                        ...
                        m.(l).(k_end) <-
                          m.(l).(k_end) -: (m.(i).(k_end) *: m_lj /: m.(i).(j));
                        if !is_only_zero && m.(l).(k_end) <>: A.zero then
                          raise Unsolvable)
                    done;
                    raise Found)
                done
      ...
      let init_with_vec v =
        ...
        new_matrix.(0) <- V.to_array v;
        new_matrix

      let reduce_col_with_vec m j v =
        if M.tracing then M.trace "arrayMatrix" "reduce_col_with_vec";
        for i = 0 to num_rows m - 1 do
      ...
      let get_pivot_positions m =
        if M.tracing then M.trace "arrayMatrix" "get_pivot_positions";
        let pivot_elements = Array.make (num_rows m) 0 in
        Array.iteri
          (fun i x -> pivot_elements.(i) <- Array.findi (fun z -> z =: A.one) x)
          m;
        pivot_elements

      let rref_vec m pivot_positions v =
        if M.tracing then M.trace "arrayMatrix" "rref_vec";
        let insert = ref (-1) in
        for j = 0 to Array.length v - 2 do
          if v.(j) <>: A.zero then
            match Array.bsearch Int.ord pivot_positions j with
            | `At i ->
              let beta = v.(j) /: m.(i).(j) in
              Array.iteri (fun j' x -> v.(j') <- x -: (beta *: m.(i).(j'))) v
            | _ ->
              if !insert < 0 then (
                let v_i = v.(j) in
                Array.iteri (fun j' x -> v.(j') <- x /: v_i) v;
                insert := j;
                reduce_col_with_vec m j v)
        done;
        if !insert < 0 then
          if v.(Array.length v - 1) <>: A.zero then None else Some m
        else
          let new_m = Array.make_matrix (num_rows m + 1) (num_cols m) A.zero in
          let i, j = Array.pivot_split Int.ord pivot_positions !insert in
          if i = 0 && j = 0 then (
            new_m.(0) <- v;
            Array.blit m 0 new_m 1 (num_rows m))
          else if i = num_rows m && j = num_rows m then (
            Array.blit m 0 new_m 0 j;
            new_m.(j) <- v)
          else (
            Array.blit m 0 new_m 0 i;
            new_m.(i) <- v;
            Array.blit m i new_m (i + 1) (Array.length m - j));
          Some new_m
      ...
      let rref_vec_with m v =
        ...
        if is_empty m then (
          ...
          | i ->
            if i = Array.length v - 1 then None
            else
              let v_i = v.(i) in
              Array.iteri (fun j x -> v.(j) <- x /: v_i) v;
              Some (init_with_vec @@ V.of_array v))
        else
      ...
      let rref_matrix_with m1 m2 =
        ...
        try
          for i = 0 to num_rows s_m - 1 do
            ...
            match res with None -> raise Unsolvable | Some res -> b := res
          done;
          Some !b
      ...
      let normalize m =
        ...
        let copy = copy m in
        if normalize_with copy then Some copy else None
      ...
      let is_covered_by m1 m2 =
        ...
        if num_rows m1 > num_rows m2 then false
        else
                ...
                for j = 0 to Array.length m1_i - 2 do
                  if m1_i.(j) <>: A.zero then
                    ...
                    | `At pos ->
                      ...
                      Array.iteri
                        (fun j' x ->
                           m1_i.(j') <- m1_i.(j') -: (beta *: m2.(pos).(j')))
                        m1_i
                    | _ -> raise Stdlib.Exit
                done;
                if m1_i.(num_cols m1 - 1) <>: A.zero then raise Stdlib.Exit)
            done;
            true
      ...
      let find_opt f m =
        ...
        let f' x = f (V.of_array x) in
        Option.map V.of_array (Array.find_opt f' m)

      let map2 f m v =
        if M.tracing then M.trace "arrayMatrix" "map2";
        let f' x y = V.to_array @@ f (V.of_array x) y in
        Array.map2 f' m (V.to_array v)

      let map2_with f m v =
        if num_rows m = V.length v then
          Array.iter2i
            (fun i x y -> m.(i) <- V.to_array @@ f (V.of_array x) y)
            m (V.to_array v)
        else
      ...
      let map2i_with f m v =
        if num_rows m = V.length v then
          Array.iter2i
            (fun i x y -> m.(i) <- V.to_array @@ f i (V.of_array x) y)
            m (V.to_array v)
        else
          for i = 0 to Stdlib.min (num_rows m) (V.length v) - 1 do
...
module SparseMatrix : AbstractMatrix =
  functor
    ...
    struct
      ...
      let empty () = Array.make_matrix 0 0 (0, A.zero)
      let num_rows m = failwith "TODO"
      let is_empty m = num_rows m = 0
      let num_cols m = failwith "TODO"
      let copy m = failwith "TODO"
      let copy m = failwith "TODO"
      ...
      let append_row m row = failwith "TODO"
      let get_row m n = failwith "TODO"
      let remove_row m n = failwith "TODO"
      let get_col m n = failwith "TODO"
      let get_col m n = timing_wrap "get_col" (get_col m) n
      let set_col_with m new_col n = failwith "TODO"

      let set_col_with m new_col n =
        timing_wrap "set_col" (set_col_with m new_col) n

      let set_col m new_col n = failwith "TODO"
      let append_matrices m1 m2 = failwith "TODO"
      let equal m1 m2 = timing_wrap "equal" (equal m1) m2
      let reduce_col_with m j = failwith "TODO"

      let reduce_col_with m j =
        timing_wrap "reduce_col_with" (reduce_col_with m) j

      let reduce_col m j = failwith "TODO"
      let del_col m j = failwith "TODO"
      let del_cols m cols = failwith "TODO"
      let del_cols m cols = timing_wrap "del_cols" (del_cols m) cols
      let map2i f m v = failwith "TODO"
      let remove_zero_rows m = failwith "TODO"
      let rref_with m = failwith "TODO"
      let rref_with m = timing_wrap "rref_with" rref_with m
      let init_with_vec v = failwith "TODO"
      let reduce_col_with_vec m j v = failwith "TODO"
      let get_pivot_positions m = failwith "TODO"
      let rref_vec m pivot_positions v = failwith "TODO"
      ...
      let normalize_with m = failwith "TODO"
      let normalize_with m = timing_wrap "normalize_with" normalize_with m
      let normalize m = failwith "TODO"
      let is_covered_by m1 m2 = failwith "TODO"
      let is_covered_by m1 m2 = timing_wrap "is_covered_by" (is_covered_by m1) m2
      let find_opt f m = failwith "TODO"
      let map2 f m v = failwith "TODO"
      let map2_with f m v = failwith "TODO"
      let map2_with f m v = timing_wrap "map2_with" (map2_with f m) v
      let map2i_with f m v = failwith "TODO"
